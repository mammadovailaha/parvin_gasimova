...
const sosialLinks = [
  ...
  { id: 4, icon: <FaWhatsapp />, link: "https://wa.me/994505547568" },
]
          ...
          <div className='flex gap-5'>
            {sosialLinks.map(link => (
              <Link
                target="_blank"
                key={link.id}
                className='w-10 h-10 border border-gray-300/60 rounded-full flex justify-center items-center hover:bg-white hover:border-white transition-all ease-in-out delay-200 text-white hover:text-black text-xl' 
                to={link.link}>
                {link.icon}
              </Link>
            ))}
